...
postNotification = db.Table(
    "postNotification",
    db.Column("not_id", db.Integer, db.ForeignKey("notification.id")),
    db.Column("posts_id", db.Integer, db.ForeignKey("post.id")),
)

# table for notifications for liked comments
commentNotification = db.Table(
    "commentNotification",
    db.Column("not_id", db.Integer, db.ForeignKey("notification.id")),
    db.Column("commnt_id", db.Integer, db.ForeignKey("comment.id")),
)

# table for notifications for dropping comment on post
commentPostNotification = db.Table(
...
commentChildNotification = db.Table(
    "commentChildNotification",
    db.Column("not_id", db.Integer, db.ForeignKey("notification.id")),
    db.Column("commnt_id", db.Integer, db.ForeignKey("comment.id")),
)

# table for notifications for following/unfollowing user
followNotification = db.Table(
    "followNotification",
    db.Column("not_id", db.Integer, db.ForeignKey("notification.id")),
    db.Column("person_id", db.Integer, db.ForeignKey("user.id")),
)
...
childCommentNotification = db.Table(
    "childCommentNotification",
    db.Column("not_id", db.Integer, db.ForeignKey("notification.id")),
    db.Column("child_commnt_id", db.Integer, db.ForeignKey("childcomment.id")),
)
...
class User(PaginatedAPIMixin, db.Model, UserMixin):
    ...
    notification = db.relationship("Notification", backref="author", lazy="dynamic")
    ...
    def get_notifications(self):
        notification_list = (
            ...
            .filter(Notification.user_id != self.id)
            .filter(
                or_(
                    Notification.post_creator_id == self.id,
    ...
    def followed_posts(self):
        ...
        reported_count = (
            db.session.query(reportedPost.c.post_id, func.count("*").label("reporting"))
            .group_by(reportedPost.c.post_id)
            .subquery()
        )
        ...
        followed = (
            ...
            .outerjoin(reported_count)
            .outerjoin(reportedPost)
            .filter(followers.c.follower_id == self.id)
            .filter(
                or_(
                    reportedPost.c.person_id != self.id,
                    reportedPost.c.person_id == None,
                )
            )
            .filter(
    ...
    def filter_posts(self):
        ...
        no_intrst_count = (
            ...
            .group_by(notInterested.c.post_id)
            .subquery()
        )
        # get count of reported posts by post id
        reported_count = (
        ...
        explore = (
            ...
            .outerjoin(no_intrst_count)
            .outerjoin(notInterested)
            .outerjoin(reported_count)
            .outerjoin(reportedPost)
            .filter(
                or_(
                    notInterested.c.person_id != self.id,
                    notInterested.c.person_id == None,
                )
            )
            .filter(
                or_(
                    no_intrst_count.c.not_interest_count < 2,
                    notInterested.c.person_id == None,
                )
            )
            .filter(
...
class Post(PaginatedAPIMixin, db.Model):
    ...
    likes = db.relationship(
        "User",
        secondary=likedPosts,
        backref=db.backref("likedPosts", lazy="dynamic"),
        lazy="dynamic",
    )
    not_interested = db.relationship(
        "User",
        secondary=notInterested,
        backref=db.backref("notInterested", lazy="dynamic"),
        lazy="dynamic",
    )
    reported = db.relationship(
        "User",
        secondary=reportedPost,
        backref=db.backref("reportedPost", lazy="dynamic"),
        lazy="dynamic",
    )
    ...
    def interested(self, user):
        return (
            self.not_interested.filter(notInterested.c.person_id == user.id).count() > 0
        )
...
class Comment(PaginatedAPIMixin, db.Model):
    ...
    likes = db.relationship(
        "User",
        secondary=likedComments,
        backref=db.backref("likedComments", lazy="dynamic"),
        lazy="dynamic",
    )
...
likedChildComments = db.Table(
    "likedChildComments",
    db.Column("comment_id", db.Integer, db.ForeignKey("childcomment.id")),
    db.Column("liker_id", db.Integer, db.ForeignKey("user.id")),
)
...
class ChildComment(PaginatedAPIMixin, db.Model):
    ...
    notifications = db.relationship(
        "Notification", backref="childcomment", lazy="dynamic"
    )
    likes = db.relationship(
        "User",
        secondary=likedChildComments,
        backref=db.backref("likedChildComments", lazy="dynamic"),
        lazy="dynamic",
    )
    timestamps = db.Column(db.DateTime, index=True, default=datetime.utcnow)
    username = db.Column(db.String(360))
...
def delete_account(user_id):
    ...
    delete_follower = delete(followers).where(followers.c.follower_id == user_id)
    db.session.execute(delete_follower)

    delete_following = delete(followers).where(followers.c.followed_id == user_id)
    ...
    delete_posts = delete(likedPosts).where(likedPosts.c.liker_id == user_id)
    db.session.execute(delete_posts)

    # Delete all liked comments
    delete_comments = delete(likedComments).where(likedComments.c.liker_id == user_id)
...
class Notification(PaginatedAPIMixin, db.Model):
    ...
    like_post = db.relationship(
        "Post",
        secondary=postNotification,
        backref=db.backref("postNotification", lazy="dynamic"),
        lazy="dynamic",
    )
    post_creator_id = db.Column(db.Integer)
    like_comment = db.relationship(
        "Comment",
        secondary=commentNotification,
        backref=db.backref("commentNotification", lazy="dynamic"),
        lazy="dynamic",
    )
    comment_creator_id = db.Column(db.Integer)
    like_child_comments = db.relationship(
        "ChildComment",
        secondary=childCommentNotification,
        backref=db.backref("childCommentNotification", lazy="dynamic"),
        lazy="dynamic",
    )
    child_comment_creator_id = db.Column(db.Integer)
    follow_notify = db.relationship(
        "User",
        secondary=followNotification,
        backref=db.backref("followNotification", lazy="dynamic"),
        lazy="dynamic",
    )
    followed_user_id = db.Column(db.Integer)
    comment_notify = db.relationship(
        "Post",
        secondary=commentPostNotification,
        backref=db.backref("commentPostNotification", lazy="dynamic"),
        lazy="dynamic",
    )
    comment_post_author_id = db.Column(db.Integer)
    child_comment_notify = db.relationship(
        "Comment",
        secondary=commentChildNotification,
        backref=db.backref("commentChildNotification", lazy="dynamic"),
        lazy="dynamic",
    )
