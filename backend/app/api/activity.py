...
from ..models.models import User, Post, Comment, ChildComment, Notification
...
def upload_file():
    ...
    upload_result = upload(
        file_to_upload, folder="Posts/", public_id=file_to_upload.filename
    )
...
def search_user():
    ...
    user = User.query.filter(
        or_(
            ...
            User.first_name == usrname,
            User.last_name == usrname,
        )
    )
    # print(user.all())
...
def get_profile_posts(username):
    ...
    response = User.to_collection_dict(
        user.posts, page, per_page, "api.get_profile_posts", username=username
    )
    return response
...
def get_home_posts(username):
    ...
    response = User.to_collection_dict(
        user.followed_posts(), page, per_page, "api.get_home_posts", username=username
    )
    return response
...
def explore():
    ...
    response = User.to_collection_dict(
        user.filter_posts(), page, per_page, "api.explore"
    )
    return response
...
def follow(username):
    ...
    if notify == True:
        ...
        new_notf = Notification(
            timestamps=follow_time,
        ...
        notific = Notification.query.filter_by(
            user_id=user.id, followed_user_id=currentUser.id
        ).first()
...
def unfollow(username):
    ...
    if notify == True:
        notific = Notification.query.filter_by(
            user_id=currentUser.id, followed_user_id=user.id
        ).first_or_404()
...
def get_comments():
    ...
    response = Post.to_collection_dict(
        post.comments, page, per_page, "api.get_comments"
    )
    return response
...
def get_child_comments():
    ...
    response = Comment.to_collection_dict(
        comment.comments, page, per_page, "api.get_child_comments"
    )
    return response
...
def likeChildComment(id):
    ...
    if comment.user_id != user.id:
        ...
        # delete notification if like action is reversed
            ...
            notific = Notification.query.filter_by(
                child_comment_id=id, user_id=user.id
            ).first_or_404()
...
def notify():
    ...
    response = User.to_collection_dict(
        user.get_notifications(), page, per_page, "api.notify"
    )
    return response
