...
from ..models.models import User, delete_account, Notification
...
def get_user(uzername):
    ...
    if request.json.get("username"):
        ...
        response = {"user": user_dict, "following": following}
    else:
        response = {
...
def get_followers(uzername):
    ...
    user_data = User.to_collection_dict(
        user.followers, page, per_page, "api.get_followers", uzername=uzername
    )
    ...
    response = {"user": user_data, "following": following}
    return response
...
def get_followed(uzername):
    ...
    user_data = User.to_collection_dict(
        user.followed, page, per_page, "api.get_followed", uzername=uzername
    )
...
def update(uzername):
    ...
    if (
        ...
        and User.query.filter_by(username=data["username"].lower()).first()
    ):
        return bad_request("username already exists")
    if (
        "email" in data
        and data["email"] != user.email
        and User.query.filter_by(email=data["email"].lower()).first()
    ):
    ...
    if avatar_image.filename:
        upload_result = upload(
            avatar_image, folder="Profile/", public_id=avatar_image.filename
        )
    ...
    if header_image.filename:
        upload_result = upload(
            header_image, folder="Profile/", public_id=header_image.filename
        )
        # and then get the url for the transitioned uploaded file and store it in the database
